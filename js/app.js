...
function renderTraits() {
    ...
    [...traitsList].sort(() => Math.random() - 0.5).forEach(t => {
        ...
        // Updated styling for light mode trait cards
        btn.className = 'trait-card rounded-xl p-3 h-24 flex flex-col items-center justify-center gap-1 text-center group cursor-pointer';
        btn.innerHTML = `
            <span class="font-bold text-xs font-sans">${t.en}</span>
            <span class="urdu text-gray-400 text-[10px]">${t.ur}</span>
        `;
